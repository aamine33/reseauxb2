  ...
  def check_url(url):
    try:
        ...
        # Petite sécurité en plus : vérifier si le domaine pointe vers une IP privée
        # (ça bloque les attaques DNS rebinding qui tentent d’accéder aux IP internes)
        if ipaddress.ip_address(socket.gethostbyname(hostname)).is_private:
            return '''
                <html>
                    <head>
                    ...
                    <body>
                        <img src="%s"/>
                        <p style="text-align:center; color:red;">No access to private IPs!</p>
                    </body>
                </html>
            ''' % (flask.url_for('static', filename='access_denied.jpg'),)
        
        # Validation du FQDN (nom de domaine complet)
        check = valid_fqdn(hostname)
    except Exception as e:
        # Si le domaine est invalide ou s'il y a une erreur, on renvoie une page d'erreur sympa
        return '''
            <html>
                <head>
                ...
                <body>
                    <img src="%s"/>
                    <p style="text-align:center; color:blue;">Are you sure about this URL?</p>
                </body>
            </html>
        ''' % (flask.url_for('static', filename='what.jpg'),)
    
    # Si le domaine n'est pas valide, on bloque
    if check == False:
        return '''
            <html>
                <head>
                ...
                <body>
                    <img src="%s"/>
                    <p style="text-align:center; color:purple;">Nice try, but no private URLs!</p>
                </body>
            </html>
        ''' % (flask.url_for('static', filename='wow-so-clever.jpg'),)
    
    # Tout va bien, on renvoie rien et on laisse continuer le traitement
    return None
